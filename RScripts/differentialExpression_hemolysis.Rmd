...
png(filename="mature_miRNA.png",width=8,height=6,units="in",res=300)
ggplot(data = rank,
       aes(x = readCounts,
           y = unique_miRs,
           colour = haemolysis,
  ...
  scale_colour_manual(values = c("black", "darkRed"),
                      labels = c("miR-23a-miR-451 \U003C 7",
                                 "miR-23a-miR-451 \U2265 7")) +
  labs(colour = paste("Haemolysis", "(","\U0394","Cq",")", sep="")) +
...
levels(col.hemo) <- brewer.pal(nlevels(col.hemo), "Set3")
col.hemo <- as.character(col.hemo)
...
saveRDS(DGElist_plasma$counts, here("cleanData/normCounts.rds"))
...
MAplot_pregnancy <- ggplot(data = MAplot_data_pregnancy, 
  ...
  theme(axis.text.x = element_text(colour = "black")) +
  theme(axis.text.y = element_text(colour = "black")) +
  geom_text_repel(data = filter(MAplot_data_pregnancy,
                                FDR_FC_Decide == 1 & logFC.x >= 2), 
                  aes(label=rowname)) +
  geom_text_repel(data = filter(MAplot_data_pregnancy,
                                FDR_FC_Decide == -1 & logFC.x <= -2),
                  aes(label=rowname)) +
  geom_text_repel(data = filter(MAplot_data_pregnancy,
                                FDR_FC_Decide == 1 & AveExpr > 10),
                  aes(label=rowname)) +
  geom_text_repel(data = filter(MAplot_data_pregnancy,
                                FDR_FC_Decide == -1 & AveExpr > 10),
                  aes(label=rowname))
...
# create the data for pairs plots
pairsDat <- subset(
  correctedCounts,
  rownames(correctedCounts) %in% top_DE_classifier$SYMBOL) %>%
  # subset(
  #   .,
  #   rownames(.) %notin% combined_mirs_plus_pregnancy$SYMBOL) %>%
  t() %>%
  as.data.frame() %>% 
  tibble::rownames_to_column("samplename") %>% 
  left_join(., DGElist_plasma$samples[, c("samplename", "haemolysis")]) %>% 
  dplyr::select(., samplename, haemolysis, everything())

# create the pairs plots
my_cols <- c("grey", "red")  
pairs(pairsDat[, 3:ncol(pairsDat)],
      pch = 19,
      cex = 0.5,
      col = my_cols[pairsDat$haemolysis],
      lower.panel = NULL)
...
dropped <- subset(top_DE_classifier, SYMBOL %notin% combined_mirs_plus_pregnancy$SYMBOL)
...
cdat <- ddply(dplyr::select(as.data.frame(cpm(DGElist_plasma$counts, log=TRUE)), plotVar) %>%
  ...
  "classifier", summarise, geometric.mean = psych::geometric.mean(get(plotVar), na.rm = TRUE))
...
dplyr::select(as.data.frame(cpm(DGElist_plasma$counts, log = TRUE)), plotVar) %>%
  ...
  labs(title = paste(plotVar, "Denisty of CPM counts (10)"),
       ...
       x = "log2 CPM Counts",
       y = "Density") +
  scale_fill_discrete(name = "Classifier \nStatus", labels = c("Non-Classifier", "Classifier", "Dropped")) +
  theme_bw(base_size = 16)
...
## all samples - 10 classifier miRs
...
```{r calculate geometric mean difference reduced classifier list}
...
distributionDifference <- lapply(varc, function(x){
  ...
  cdat <- ddply(
    dplyr::select(
      as.data.frame(
        cpm(DGElist_plasma$counts, log = TRUE)), x) %>%
      tibble::rownames_to_column("mirna") %>% 
      mutate(., classifier = as.factor(ifelse(mirna %in% combined_mirs_plus_pregnancy$SYMBOL, 1,
                                              ifelse(mirna %in% dropped$SYMBOL, 2,
                                              ifelse(mirna %notin% top_DE_classifier$SYMBOL, 0, NA))))),
...
plotData_disDiff_dCq <- full_join(
  unlist_distributionDifference,
  DGElist_plasma$samples[, c("samplename", "miR23a.miR451a", "haemolysis")]
  ) %>% 
  dplyr::mutate(fineClass = ifelse(
    miR23a.miR451a < 7.0, "noDetection",
    ifelse(
      miR23a.miR451a == 7, "borderline",
      ifelse(
        miR23a.miR451a > 7, "haemolysed",
        NA
      )
    )
  ))

correlationTest <- cor.test(plotData_distDiff_dCq$distributionDifference,plotData_distDiff_dCq$miR23a.miR451a, method=c("pearson"))

# write_delim(plotData_disDiff_dCq, here("shinyVamp/inputData/plotData_distDiff_dCq.csv"), delim = ",")

# only include the final classifier miRs (ie take the pregnancy ones out)
png(file="correlation.png",width=8,height=6,units="in",res=300)
ggplot(data = plotData_disDiff_dCq,
       aes(x = .data$miR23a.miR451a,
           y = .data$distributionDifference,
           colour = .data$fineClass,
  ...
  geom_point() +
  scale_colour_manual(breaks = c("noDetection", "haemolysed", "borderline"), 
                       values=c("black", "darkRed", "darkGrey"),
                      labels = c("No Detection", "Haemolysed", "Borderline")) +
  geom_text_repel(data = filter(plotData_disDiff_dCq,
                  haemolysis == "none" &
                  distributionDifference >= 1.9),
    box.padding = 1,
    aes(label = samplename),
    show.legend = FALSE
    ) +
  ...
  #                                                      plotData_disDiff_dCq,
  #                                                      function(x) min(x))[2, 2]),
  #                 box.padding = 1,
  #                 aes(label = samplename)) +
  
  # show a tick mark every 1 from -5 t0 10
  scale_x_continuous(name = paste("\U0394", "Cq (miR23a-miR451a)",sep=""),
                     breaks = seq(-5, 10, 1)) +
  labs(
      # title = "Distribution difference using final classifiers",
      # subtitle = "based on three classification groups",
      x = paste("\U0394", "Cq (miR23a-miR451a)",sep=""),
      y = "Distribution Difference",
      colour = paste("\U0394", "Cq (miR23a-miR451a) ",sep="")
      ) +
  theme_bw(base_size = 16) +
...
ggplot(data = plotData_disDiff_dCq) +
  ...
  labs(
    # title = "Distribution difference using final classifiers",
    # subtitle = "based on three classification groups",
    legend = "Haemolysis metric"
    ) +
  theme(legend.position = "bottom") +
  theme_bw(base_size = 16)

# plot as histogram side by side
png(file="distribution.png",width=8,height=6,units="in",res=300)
ggplot(
  data = plotData_disDiff_dCq,
  aes(
  ...
  scale_fill_manual(values = c("black", "darkRed"),
                    name = paste("\U0394", "Cq (miR23a-miR451a)",sep=""),
                    labels = c(paste("\U0394", "Cq (miR23a-miR451a) ","\U003C 7",sep=""),
                               paste("\U0394", "Cq (miR23a-miR451a) ","\U2265 7",sep=""))) +
  scale_colour_manual(values = c("black", "darkRed"),
                      name = paste("\U0394", "Cq (miR23a-miR451a)",sep=""),
                      labels = c(paste("\U0394", "Cq (miR23a-miR451a) ","\U003C 7",sep=""),
                                 paste("\U0394", "Cq (miR23a-miR451a) ","\U2265 7",sep=""))) +
